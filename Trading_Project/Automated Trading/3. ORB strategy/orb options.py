...
while True:
	for name in watchlist:
		...
		if (ltp > range_high) and (name not in traded):
			option_name = taf.option_name_finder(multiplier=0, name=name, exipry="28OCT21", ce_pe="CE")
			print("Buy order placed for", name, option_name)
			orderparams = {"variety": 'NORMAL', "tradingsymbol": option_name, "symboltoken": taf.get_token_and_exchange(option_name)[0], "transactiontype": 'BUY', "exchange": 'NFO', "ordertype": 'MARKET', "producttype": 'INTRADAY', "duration": 'DAY', "price": 0.0, "squareoff": 0.0, "stoploss": 0.0, "triggerprice": 0.0, "trailingstoploss": 0.0, "quantity": 1}
			orderId = taf.place_order(orderparams)
			traded.append(name)

		if (ltp < range_low) and (name not in traded):
			option_name = taf.option_name_finder(multiplier=0, name=name, exipry="28OCT21", ce_pe="PE")
			print("Sell order placed for", name, option_name)
			orderparams = {"variety": 'NORMAL', "tradingsymbol": option_name, "symboltoken": taf.get_token_and_exchange(option_name)[0], "transactiontype": 'BUY', "exchange": 'NFO', "ordertype": 'MARKET', "producttype": 'INTRADAY', "duration": 'DAY', "price": 0.0, "squareoff": 0.0, "stoploss": 0.0, "triggerprice": 0.0, "trailingstoploss": 0.0, "quantity": 1}
			orderId = taf.place_order(orderparams)
			traded.append(name)
